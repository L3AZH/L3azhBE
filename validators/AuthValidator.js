...
module.exports = {
  ...
  loginValidation: [
    body("ma").trim().notEmpty().withMessage("Vui long dien ma"),
    body("matkhau").trim().notEmpty().withMessage("Vui long nhap mat khau"),
    body("role")
      .trim()
      .notEmpty()
      .withMessage("Vui long chon role")
      .custom(async (value) => {
        if (!(value === "SV")) {
          if (!(value === "GV")) {
            return Promise.reject("Role khong hop le (2 role: SV/GV)");
          }
        }
      }),
  ],
